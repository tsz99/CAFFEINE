...
namespace CAFFEINE.Services
{
    public class CaffService
    {
        ...
        {
            Caption caption = new Caption() { Text="newCaption"};
            Tag tag = new Tag() { Text = "newTag" };
            Comment comment = new Comment() { Text = "newComment" };
            Ciff ciff = new Ciff() { Pixels = new byte[] { 0x20, 0x20, 0x20, 0x20, 0x20, 0x20, 0x20 },Tags = new List<Tag>() { tag }, Captions = new List<Caption>() { caption }, Width =200, Height=200,Duration=10 }; 
